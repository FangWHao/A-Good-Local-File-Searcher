...
bool is_ntfs(const char *drv)
{
	...
	if (GetVolumeInformationA((std::string(drv) + ":\\").c_str(), 0, 0, 0, &MaximumComponentLength, 0, FileSystemName, MAX_PATH + 1) && 0 == strcmp(FileSystemName, "NTFS"))
		ret = 1;
	CloseHandle(hVol);
	return ret;
}
USN GET_USN(const char *drvname, USN startUSN, bool flag, vector<int> &rm, vector<dat> &cr, vector<string> &new_name, int *renamed)
{
	...
	char FileSystemName[MAX_PATH + 1];
	DWORD MaximumComponentLength;
	if (GetVolumeInformationA((std::string(drvname) + ":\\").c_str(), 0, 0, 0, &MaximumComponentLength, 0, FileSystemName, MAX_PATH + 1) && 0 == strcmp(FileSystemName, "NTFS")) // 判断是否为 NTFS 格式
	{
