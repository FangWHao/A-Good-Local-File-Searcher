...
class project : private NFA, private DFA
{ //整个工程使用一个class，可调用正则表达式读入和匹配两个函数
public:
...
int get_next_pos(char *reg, int st)
...
bool NFA::read_reg(char *reg, int L, int R, int start_point, int dest_point, bool is_birth)
{
	...
	{			   //排除运算，压入隐边队列，并添加所有排除字符进入charpool集
		/********/ //  最新完成部分，不保证正确
		hidden_edge_node current_edge;
...
{ //拼写检查主函数
	...
	for (int i = 0; i <= len - 1; i++)
	{
		...
		if (reg[i] == '*' && (i == 0 || ((reg[i - 1] == '|' || reg[i - 1] == '(' || reg[i - 1] == '*') && (i - 1 == 0 || reg[i - 2] != '\\'))))
		{
			puts("The position of * is wrong!");
			return false;
		}
		if (reg[i] == '|' && ((i == 0 || ((reg[i - 1] == '|' || reg[i - 1] == '(') && (i - 1 == 0 || reg[i - 2] != '\\'))) || (i == len - 1 && reg[i + 1] == ')')))
		{
			puts("The position of | is wrong!");
			return false;
		}
...
*/
set<int> get_closure(NFA &bas_nfa, set<int> &bas_set, char key0)
{ //寻找bas_set的key0闭包的函数（e闭包为'\0'闭包）
	set<int> ret;
	set<int>::iterator iter;
...
set<int> get_closure(NFA &bas_nfa, set<int> &bas_set, char key0, char key1)
{ //寻找双字符闭包的函数（未使用）
	set<int> ret;
	set<int>::iterator iter;
	pair<set<int>::iterator, bool> getret;
...
bool compset(set<int> x, set<int> y)
{ //判断两个set是否相等
	if (x.size() != y.size())
		return false;
	set<int>::iterator iter;
...
void DFA::transform_NFA_to_DFA(NFA &bas_nfa)
{ //转换函数
	size = 0;
	set<int> tmpset;
	set<char>::iterator charpool_iter;
