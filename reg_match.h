...
class project: public NFA, public DFA {
public: 
...
//	cout<<start_point<<' '<<dest_point<<' '<<L<<' '<<R<<endl;
//	cout<<ret.mode<<' '<<ret.leftL<<' '<<ret.leftR<<' '<<ret.rightL<<' '<<ret.rightR<<endl;
...
//		cout<<"LBBB   "<<start_point<<' '<<dest_point<<endl;
	}
	else if(ret.mode == 1) {
		int mid = ++size;
